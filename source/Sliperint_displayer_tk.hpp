...
const char * get_state(void * d) {
    static char * r;
    const Sliperint_displayer &displayer = *((Sliperint_displayer*)d);
    if (displayer.success_value == -1) {
        return "Searching...";
    } else {
        asprintf(&r, "Found the solution in %d steps of descend!", displayer.success_value);
        return r;
    }
}
